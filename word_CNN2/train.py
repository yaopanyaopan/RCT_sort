...
chinese_copurs = True
if chinese_copurs:
    # train,dev,label_train,label_dev,max_word_size , W_embedding = input_helpers.getDataSets()
    train, dev, label_train, label_dev, max_word_size, W_embedding = input_helpers.getsinomedDataSets()
...
with tf.Session() as sess:
        ...
        def train_step(x1_batch, y_batch,epoch):
            ...
            _,step,loss, scores,score1  ,W ,word ,accuracy = sess.run(
                [tr_op_set, global_step, Model.loss,Model.scores,Model.score1,Model.W,Model.word_embedding,Model.accuracy],feed_dict)
        ...
        to_picture(title='dev-loss', x_content=epoch, y_content=dev_loss, xlabel='Epoch', ylabel='loss', xlim=(0,int(Flags.num_epochs)) ,ylim=(0,0.5),xticks=np.linspace(0,5,6,endpoint=True),yticks=np.linspace(0,1,21,endpoint=True),
                   path=out_dir+'/'+'dev-loss.png')
