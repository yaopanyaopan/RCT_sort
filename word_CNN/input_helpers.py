...
class InputHelper(object):
    ...
    def batch_iter(data,batch_size, only_part):
        ...
        shuffle=True
        data_size = len(data)
        num_batches_per_epoch = int(len(data)*0.5/ batch_size) + 1
...
def NDCG_1(n_rank,n_sco):
    ...
    # error = []
    # top = 723
    # for index_i,i in enumerate(reli[:723]):
    #     if i!=5.0:
    #         error.append([index_i,i])
    #     if (i == 5.0  ) and index_i < top:
    #         count_top +=1
    # print('准确率', float(count_top) / top)
    # print(count_top)
    #
    # error = []
    # for index_i,i in enumerate(reli[723:990]):
    #     if i!=4.0:
    #         error.append([index_i+723,i])
    #     # if (i == 5.0 or i == 4.0 ) and index_i + 723 < top:
    #     #     count_top +=1
    # error = []
    # for index_i,i in enumerate(reli[990:1096]):
    #     if i!=3.0:
    #         error.append([index_i+990,i])
    #     if (i == 5.0 or i == 4.0 ) and index_i + 990 < top:
    #         count_top +=1
    # error = []
    # for index_i,i in enumerate(reli[1096:2246]):
    #     if i!=2.0:
    #         error.append([index_i+1096,i])
    #     # if (i==5.0 or i==4.0 or i==3.0 or i==2.0) and index_i+1096 < top:
    #     #     count_top +=1
    # error = []
    # for index_i,i in enumerate(reli[2246:]):
    #     if i!=1.0:
    #         error.append([index_i+2246,i])
    ...
    sum_D = 0.0
    for index,val in enumerate(reli[:2000]):
        sum_D = sum_D + float(math.pow(2,val)-1)/math.log((index+1)+1,2)
        # sum_D += float(val) / math.log((index + 1) + 1, 2)
    # print(sum_D)
    # res = sorted(reli,reverse=True)
    res = sorted(n_rank, reverse=True)
    # print(res)
    sum_I = 0.0
    for index,val in enumerate(res[:2000]):
        sum_I =sum_I+ float(math.pow(2,val)-1)/math.log((index+1)+1,2)
    # print(sum_I)

    count = n_rank.count(5.0)
    count_rct = 0
...
def getDataSets():
    ...
    with open("../data/test_W.pickle","wb") as f3:
        pickle.dump(test,f3)


    with open("../data/label_test_W.pickle","wb") as f4:
        res_labels = []
        for i in label_test:
                if i == 1.0:
