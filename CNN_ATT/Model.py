...
class LSTM_Attention(object):
    ...
    def __init__(self,
        ...
        with tf.name_scope("word_out"):
            ...
            self.word_level_output1, self.num_filters_total1 = self.CNN(self.embeded_chars1, self.embedding_size, self.dropout_keep_prob,self.max_sent_len,self.max_seq_len ,self.hidden_units,name="word")
            self.word_level_output2, self.num_filters_total2 = self.CNN(self.embeded_chars2, self.embedding_size, self.dropout_keep_prob,self.max_sent_len,self.max_seq_len , self.hidden_units, name="word")
        ...
        with tf.name_scope("sentence_out"):
            ...
            self.sentence_level_output1 ,self.alphas11 = Myattention.attention(self.word_level_output1, self.max_seq_len, self.num_filters_total1, return_alphas=True,name="sentence_level" ,input_x=self.input_x1)
            self.sentence_level_output2 ,self.alphas22 = Myattention.attention(self.word_level_output2, self.max_seq_len , self.num_filters_total2, return_alphas=True,name="sentence_level",input_x=self.input_x2)

        with tf.name_scope("full_connect"):
            self.w_all = tf.get_variable(initializer=initializer,shape=[self.num_filters_total1,1] ,dtype=tf.float32,name="w_all")
