...
class InputHelper(object):
    ...
    def batch_iter(data,batch_size, only_part):
        ...
        if shuffle==True:
                ...
                if only_part==True:
                    for shuffle_indice in shuffle_indices[:int(data_size*0.1)]:  # 每个epoch只随机取一部分数据
...
def presinomed_process():
    ...
    first_line = []
    with open("../sinomed/sinomed_RCT.csv","r",encoding="utf8") as fr:
        count = 0
        reader = csv.reader(fr)
        for i in reader:
            if(count==0):
            ...
            for word in ab:
                word_freq[word] += 1
        with open("../sinomed/sinomed_noneRCT.csv", "r", encoding="utf8") as fr:
            count = 0
            reader = csv.reader(fr)
            for i in reader:
                if (count == 0):
        ...
        with open("../sinomed/sinomedword_freq.csv","w",encoding='utf8') as fw:
            res = []
            for k,v in word_freq.items():
                res.append([k,v])
    ...
    DD = [i for i in range(len(data))]
    random.shuffle(DD)
    with open("../sinomed/sinomed_RCT_new.csv","w") as fw:
        writer = csv.writer(fw)
        writer.writerow(first_line)
        for i in DD:
            writer.writerow(data[i])
    DD = [i for i in range(len(data_none))]
    random.shuffle(DD)
    with open("../sinomed/sinomed_noneRCT_new.csv","w") as fw:
        writer = csv.writer(fw)
        writer.writerow(first_line)
        for i in DD:
            writer.writerow(data_none[i])
    ...
    with open("../data/label_test.pickle","wb") as f4:
        ...
        for i in label_test:
                if i == 1.0:
                    ...
                    res_labels.append(2.0)
                elif i == 3.0:
                    res_labels.append(4.0)
                elif i == 2.0:
                    res_labels.append(3.0)
                else:
                    res_labels.append(5.0)
...
def getDataSets_svm():
    ...
    print('start...')
    vectorizer = sklearn.feature_extraction.text.TfidfVectorizer(analyzer="word",binary=False,ngram_range=(1,3))
    t_data = vectorizer.fit_transform(data_set)
    # test = vectorizer.fit_transform(test)
    # train = vectorizer.fit_transform(train)

    train, test, label_train, label_test = sklearn.model_selection.train_test_split(
        t_data, labels, test_size=0.2999, random_state=20, shuffle=False)

    # print(vectorizer.get_feature_names())
    # print(len(vectorizer.get_feature_names()))

    # counter = sklearn.feature_extraction.text.CountVectorizer(analyzer="word",ngram_range=(1,1))
    # t_data = counter.fit_transform(data_set)
    # print(counter.get_feature_names())
    # print(counter.vocabulary_)
    # print(len(counter.vocabulary_))

    # sc = sklearn.preprocessing.StandardScaler()
    # t_data = sc.fit_transform(t_data)

    print('over...')

    with open("svm_test.pickle","wb") as f3:
        pickle.dump(test,f3)

    with open("svm_label_test.pickle","wb") as f4:
        pickle.dump(label_test,f4)

    return train,label_train,test,label_test

def getsinomedDataSets_svm():
    ...
    with open("../sinomed/sinomed_RCT_new.csv", 'r', encoding='utf8') as fr:
        ...
        for i in reader:
            if count==0:
                ...
                continue
            sentence = []
            result = jieba.lcut(i[3])  # 摘要
            labels.append(1.0)          # 是RCT
            for j in result:
                if j in vocab.keys():
                    sentence.append(j)
            avg_word_size.append(len(sentence))
    ...
    # train, test, label_train, label_test = sklearn.model_selection.train_test_split(
    #     t_data, labels, test_size=0.2999, random_state=20, shuffle=True)   # chinese_corpus,5
    train, test, label_train, label_test = sklearn.model_selection.train_test_split(
        t_data, labels, test_size=0.3, random_state=20, shuffle=True)  # sinomed
    ...
    with open("sinomed_svm_test.pickle","wb") as f3:
        pickle.dump(test,f3)

    with open("sinomed_svm_label_test.pickle","wb") as f4:
        pickle.dump(label_test,f4)
...
def getDataSets_english_svm():
    ...
    with open("../data/english_total_data_new.csv", 'r', encoding='utf8') as fr:
        ...
        for i in reader:
            ...
            # if time_count==1001:
            #     break
            result = nltk.tokenize.word_tokenize(i[2].strip().lower())
            # print(result)
            res = []
            for term in result:
                if term in vocab:
            ...
            # print(' '.join(res))
            data_set.append(' '.join(res))
    data_set = data_set
    print('start...')
    # vectorizer = sklearn.feature_extraction.text.TfidfVectorizer(analyzer="word",stop_words='english',binary=False,ngram_range=(1,3))
    # t_data = vectorizer.fit_transform(data_set)
    # print(t_data.shape)
    ...
    counter = sklearn.feature_extraction.text.CountVectorizer(analyzer="word",stop_words='english',ngram_range=(1,3))
    t_data = counter.fit_transform(data_set)
    # print(counter.get_feature_names())
    # print(counter.vocabulary_)
    # print(len(counter.vocabulary_))
    ...
    train, test, label_train, label_test =sklearn.model_selection.train_test_split(
        t_data, labels, test_size=0.2, random_state=20,shuffle=True)

    # print(train[0])
    # print(label_train[0])

    with open("svm_english_test.pickle","wb") as f3:
        pickle.dump(test,f3)

    with open("svm_english_label_test.pickle","wb") as f4:
        pickle.dump(label_test,f4)
