...
with tf.Session() as sess:
        ...
        def train_step(x1_batch, y_batch,epoch):
            ...
            _,step,loss, scores,score1,W ,word = sess.run(
                [tr_op_set, global_step, Model.loss,Model.scores,Model.score1,Model.W,Model.word_embedding],feed_dict)
        ...
        to_picture(title='dev-loss', x_content=epoch, y_content=dev_loss, xlabel='Epoch', ylabel='loss',
                   xlim=(0, int(Flags.num_epochs)), ylim=(0, 1), xticks=np.linspace(0, 10, 11, endpoint=True),
                   yticks=np.linspace(0, 1, 21, endpoint=True), path=out_dir + '/' + 'dev-loss.png')
        to_picture(title='dev-acc', x_content=epoch, y_content=dev_acc, xlabel='Epoch', ylabel='acc',
                   xlim=(0, int(Flags.num_epochs)), ylim=(0, 1), xticks=np.linspace(0, 10, 11, endpoint=True),
                   yticks=np.linspace(0, 1, 21, endpoint=True),
                   path=out_dir + '/' + 'dev-acc.png')
        to_picture(title='loss', x_content=epoch, y_content=train_loss, xlabel='Epoch', ylabel='loss',
                   xlim=(0, int(Flags.num_epochs)), ylim=(0, 1), xticks=np.linspace(0, 10, 11, endpoint=True),
                   yticks=np.linspace(0, 1, 21, endpoint=True),
