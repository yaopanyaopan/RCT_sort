...
pkl_x_raw1 = open('../data/test_5.pickle', 'rb')
x_test = pickle.load(pkl_x_raw1)
pkl_y_test = open('../data/label_test_5.pickle', 'rb')
...
with tf.Session() as sess:
     checkpoint_file = tf.train.get_checkpoint_state('runs/1560411017.1418657')
     ...
     for path in checkpoint_file.all_model_checkpoint_paths:
            ...
            for db in batches:
                ...
                batch_predictions ,batch_attention,batch_scores = sess.run(
                    [predictions,attention,scores],{input_x:x_test_batch,dropout_keep_prob:1.0}
                )
