...
with tf.Session() as sess:
     checkpoint_file = tf.train.get_checkpoint_state('runs/1560260242.5343494')
     max_value = 0.0
     for path in checkpoint_file.all_model_checkpoint_paths:
            ...
            for db in batches:
                ...
                for index,i in enumerate(batch_predictions):
                    ...
                    if max_value < i[0]:
                        max_value = i[0]
                        last_test = x_test_batch[index]
                        batch_attention = np.array(batch_attention)
                        last_attention = batch_attention[0][index]
            ...
            if visualize == True:
                ...
                if visualize == True:
                    ...
                    for k in range(len(last_attention)):
                        ...
                        f.write('%s<span style="margin-left:3px;background-color:rgba(0,0,0,%s)">%s 。</span>' % (k+1,last_attention[k], ''.join(string_a[k])))
                    f.write('</p>\n')
                    f.write('</div>\n')
                f.write('</body></html>')
                f.close()
