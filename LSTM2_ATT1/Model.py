...
class LSTM_Attention(object):
    ...
    def __init__(self,
        ...
        with tf.name_scope("word_out"):
            ...
            self.word_out1,_ = self.Bi_LSTM(self.embeded_chars1, self.dropout_keep_prob,self.max_sent_len, self.hidden_units,name="word",input_x=self.input_x1)
            self.word_out2,_ = self.Bi_LSTM(self.embeded_chars2, self.dropout_keep_prob,self.max_sent_len, self.hidden_units,name="word",input_x=self.input_x2)
            ...
            self.word_level_output2, self.alphas2 = Myattention.attention(self.word_out2, self.max_sent_len, self.hidden_units*2,return_alphas=True,name="word_level",input_x=self.input_x2)
        ...
        # with tf.variable_scope("sentence_out"):
            ...
            self.sentence_input2 = tf.reshape(self.word_level_output2,[-1, self.max_seq_len, self.hidden_units*2])

            self.sentence_output1 , self.sentence_output1_last = self.Bi_LSTM(self.sentence_input1, self.dropout_keep_prob,self.max_seq_len,hidden_units,name="sentence",input_x=self.input_x1)
            self.sentence_output2 , self.sentence_output2_last = self.Bi_LSTM(self.sentence_input2, self.dropout_keep_prob,self.max_seq_len,hidden_units,name="sentence",input_x=self.input_x2)
        ...
        with tf.name_scope("full_connect"):
            ...
            with tf.name_scope("score1"):
                ...
                self.score1 = tf.nn.xw_plus_b(self.sentence_output1_last, self.w_all,self.b_all,name='score1')
            with tf.name_scope("score2"):
                self.score2 = tf.nn.xw_plus_b(self.sentence_output2_last, self.w_all, self.b_all, name='score2')
