...
class LSTM_Attention(object):
    ...
    def __init__(self,
        ...
        # with tf.variable_scope("sentence_out"):
            self.sentence_input1 = tf.reshape(self.word_out1_final,[-1, self.max_seq_len, self.hidden_units*2])
            self.sentence_input2 = tf.reshape(self.word_out2_final,[-1, self.max_seq_len, self.hidden_units*2])

            self.sentence_output1 ,_ = self.Bi_LSTM(self.sentence_input1, self.dropout_keep_prob,self.max_seq_len,hidden_units,name="sentence",input_x=self.input_x1)
            self.sentence_output2 ,_= self.Bi_LSTM(self.sentence_input2, self.dropout_keep_prob,self.max_seq_len,hidden_units,name="sentence",input_x=self.input_x2)
